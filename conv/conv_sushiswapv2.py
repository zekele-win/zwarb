...
if __name__ == '__main__':
    ...
    inst = Inst(sushiswapv2_pair_usdc_eth, weth_addr, usdc_addr)
    conv_usdc_amount = inst.get_out_amount(conv_eth_amount)
    print(f'conv_usdc_amount - {conv_usdc_amount}')
    _, pool_usdc_amount = sushiswapv2_router02.functions.getAmountsOut(
    ...
    print(f'conv_usdc_amount - {conv_usdc_amount}')
    inst = Inst(sushiswapv2_pair_usdc_eth, usdc_addr, weth_addr)
    conv_eth_amount = inst.get_out_amount(conv_usdc_amount)
    print(f'conv_eth_amount - {conv_eth_amount}')
    _, pool_eth_amount = sushiswapv2_router02.functions.getAmountsOut(
