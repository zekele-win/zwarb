...
class Inst:
    ...
    def invoke_transfer_eth_to_token(self, input, output):
        return handles_input_approve(self.w3, sushiswapv2_router02_addr, eth_addr, input) + [
            sdata_maker_handle_call.make(
                caddr = sushiswapv2_router02_addr,
    ...
    def invoke_transfer_token_to_eth(self, input, output):
        return handles_input_approve(self.w3, sushiswapv2_router02_addr, self.input_token_addr, input) + [
            sdata_maker_handle_call.make(
                caddr = sushiswapv2_router02_addr,
    ...
    def make_conv(self):
        return conv_sushiswapv2.Inst(
            pair = self.w3.eth.contract(address=self.pair_addr, abi=load_abi('sushiswap-v2/SushiswapV2Pair')),
